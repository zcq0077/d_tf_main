...
#             pdb.set_trace()
    ...
    def _embed_indexes(self, idxs, pos_emb=None):
        """Embed four-hot indexes with the original attribute embeddings."""
        if pos_emb is None:
            pos_emb = self.pos_emb

        seqlen = idxs.size(1)
        lat_embeddings = self.lat_emb(idxs[:,:,0])
        lon_embeddings = self.lon_emb(idxs[:,:,1])
        sog_embeddings = self.sog_emb(idxs[:,:,2])
        cog_embeddings = self.cog_emb(idxs[:,:,3])
        token_embeddings = torch.cat(
            (lat_embeddings, lon_embeddings, sog_embeddings, cog_embeddings),
            dim=-1,
        )
        position_embeddings = pos_emb[:, :seqlen, :]
        return token_embeddings + position_embeddings

    def _encode_indexes(self, idxs):
        token_embeddings = self._embed_indexes(idxs)
        fea = self.drop(token_embeddings)
        fea = self.blocks(fea)
        fea = self.ln_f(fea)
        return fea
    ...
    def forward(self, x, masks = None, with_targets=False, return_loss_tuple=False):
        ...
        fea = self._encode_indexes(inputs) # (bs, seqlen, n_embd)
        logits = self.head(fea) # (bs, seqlen, full_size) or (bs, seqlen, n_embd)
        
        lat_logits, lon_logits, sog_logits, cog_logits = self._split_logits(logits)
